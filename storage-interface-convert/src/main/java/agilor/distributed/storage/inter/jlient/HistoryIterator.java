...
public class HistoryIterator extends AWidget implements Iterator<Val> {
    ...
    public HistoryIterator(Target target,long start,long end,long step) throws TException {



        agilor.getClient().open();

        hwnd = agilor.getClient().QueryTagHistory(target.getName(), start / 1000, end / 1000, step);
        if(hwnd<0) isEnd=true;
    }

    public HistoryIterator(Target target) throws TException {
