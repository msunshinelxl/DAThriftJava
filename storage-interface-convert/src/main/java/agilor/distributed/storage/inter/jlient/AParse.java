...
public class AParse {
    ...
    public static Val parse(TAGVAL data) {
        ...
        Val val = parse(data.value,ValType.value((char)data.type));
        Calendar c = Calendar.getInstance();
        c.setTimeInMillis(data.timestamp*1000);
        val.setTime(c);
        return val;
    }
    ...
    public static TAGVAL parse(Val data)
    {
        ...
        VAL.setType((short)data.getType().value());
    ...
    public static TAGNODE parse(Target data) {
        ...
        r.setName(data.getName());
        r.setType((short) data.getValue().getType().value());
        ...
        r.setCreationDate(data.getDateCreated().getTimeInMillis() / 1000);
        ...
        r.setId(data.getId());
        r.setInterMethod(data.isInterMethod());
    ...
    public static Target parse(TAGNODE data)
    {
        ...
        r.setValue(parse(data.value, ValType.value((char) data.type)));
        r.setSourceTag(data.getName());
        r.setDeviceName(data.getDeviceName());
        r.setAlarmHi(data.getAlarmHi());
        r.setAlarmHiHi(data.getAlarmHiHi());
